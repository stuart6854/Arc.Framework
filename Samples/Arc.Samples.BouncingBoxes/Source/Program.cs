...
Runtime.Run(new BouncingBoxesModule());

internal class BouncingBoxesModule : IRuntimeModule
{
    private static ILogger Logger { get; } = LoggerFactory.GetLogger(nameof(BouncingBoxesModule));
    ...
    {
        Platform.Initialize();
        _window = Platform.Create("Arc.Samples.BouncingBoxes", 1280, 720);
        ...
        Logger.Info($"{nameof(BouncingBoxesModule)} initialized.");
        return true;
    }
    ...
    public void OnShutdown()
    {
        ...
        Logger.Info($"{nameof(BouncingBoxesModule)} shutdown.");
    }
}